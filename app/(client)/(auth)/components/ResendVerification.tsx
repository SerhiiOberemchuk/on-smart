...
export default function ResendVerification({ email, redirect }: { email: string; redirect: string }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3 rounded-sm border border-stroke-grey p-4">
      <p className="helper_text">La tua email non è ancora confermata.</p>
      <input type="hidden" name="email" value={email} />
      <input type="hidden" name="redirect" value={redirect} />
      {state.errorMessage && <p className="helper_text text-green-400">{state.errorMessage}</p>}
